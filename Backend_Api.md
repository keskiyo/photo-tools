...
- **Инструменты и служебные эндпоинты — `POST`**; `GET` только у `/api/auth/get-session`, `/api/jobs/[id]` (статус задачи), `/api/download` (проксирование скачивания) и `/api/health`; `PATCH` — у `/api/profile`. POST-эндпоинт по ссылке в браузере открыть нельзя — будет `405`.
...
- [Скачивание результата](#скачивание-результата)
...
Лимиты заданы в [`src/config/limits.ts`](src/config/limits.ts):
...
**Администрирование:** подключён admin-плагин Better Auth. У пользователя есть `role` (`user`/`admin`) и `banned`. Страница `/admin` (доступна только админам, проверка `isAdminUser` на сервере) показывает список пользователей и позволяет блокировать/разблокировать аккаунты; действия идут через стандартные admin-эндпоинты Better Auth внутри `/api/auth/*`.
...
Отправляет 6-значный код подтверждения email через Resend. Язык письма выбирается по заголовкам запроса (cookie языка / `Accept-Language`).
...
Обновляет имя и email текущего пользователя. При смене email сбрасывает `emailVerified` и отправляет новый код подтверждения (язык письма — по заголовкам запроса).

**Требования:** активная сессия Better Auth.
...
| Код   | Когда                            | Тело                                                                       |
| ----- | -------------------------------- | -------------------------------------------------------------------------- |
| `200` | профиль обновлён                 | `{ "ok": true, "emailVerificationRequired": boolean, "email", "name" }`    |
| `400` | невалидное тело                  | `{ "error": "Invalid profile." }`                                          |
| `401` | нет сессии                       | `{ "error": "Unauthorized." }`                                             |
| `409` | email занят другим пользователем | `{ "error": "Email is already used." }`                                    |

```bash
curl -X PATCH http://localhost:3000/api/profile \
  -H "Content-Type: application/json" \
  -H "Cookie: better-auth.session_token=..." \
  -d '{"name":"Maks","email":"new@example.com"}'
```
...
Проксирует файл результата и отдаёт его как attachment (принудительное скачивание с нужным именем). Разрешены только источники из белого списка (`src/lib/download-source.ts`) — same-origin и настроенное хранилище; произвольные URL отклоняются (защита от SSRF).

**Query-параметры:**

| Параметр   | Обяз. | Назначение                                          |
| ---------- | ----- | ---------------------------------------------------- |
| `url`      | да    | URL файла (same-origin или публичное хранилище)     |
| `filename` | нет   | Имя сохраняемого файла (санитизируется)             |

| Код   | Когда                          | Тело / поведение                            |
| ----- | ------------------------------ | -------------------------------------------- |
| `200` | успех                          | поток файла, `Content-Disposition: attachment` |
| `400` | нет `url` / источник не в белом списке | `{ "error": "..." }`                  |
| `502` | источник не отдал файл         | `{ "error": "Could not fetch file." }`       |

```bash
curl -OJ "http://localhost:3000/api/download?url=/generated/ai_gen-123.jpg&filename=result.jpg"
```
...
Таблицы Better Auth: `user`, `session`, `account`, `verification`. В `user` дополнительно: `role` (`user`/`admin`, default `user`) и `banned` (default `false`) — для admin-плагина. Таблица `verification` также используется кастомными кодами подтверждения email и сброса пароля (хранятся как hash).
