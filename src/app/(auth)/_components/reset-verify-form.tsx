...
export function ResetVerifyForm({ initialEmail }: ResetVerifyFormProps) {
	...
	const { register, handleSubmit, getValues } = useForm<ResetVerifyValues>({
		defaultValues: {
			email: initialEmail,
			code: '',
		},
	...
	async function onSubmit(values: ResetVerifyValues) {
		const parsed = resetVerifySchema.safeParse(values)
		if (!parsed.success) {
			toast.error(t('auth.reset.invalid'))
			return
		}
		...
		try {
			...
			if (!response.ok || !data?.token) {
				throw new Error(getResetErrorMessage(data?.error, t))
			}
				...
				>
					...
					<label className='grid gap-2 text-sm font-semibold'>
						...
						<input
							...
							maxLength={6}
							placeholder={t('auth.reset.code.placeholder')}
