...
import { createAiDownloadFileName } from './ai-download-file-name'
...
export function AiGeneratorForm() {
	...
	async function onSubmit(values: GeneratorValues) {
		...
		try {
			...
			const nextResultUrl = await resolveJobResult(data, {
			...
			toast.error(error instanceof Error ? error.message : t('tool.ai.failed'))
		} finally {
	...
	return (
		<div className="grid items-start gap-8 lg:grid-cols-[0.95fr_1.05fr]">
			<form
				...
				className="app-surface h-fit space-y-6 rounded-(--radius-card) p-6"
			>
				...
				<div className="grid gap-4 sm:grid-cols-2">
					<label className="grid gap-2 text-sm font-semibold">
						...
						>
							<option value="none">{t('tool.ai.style.none')}</option>
							<option value="product">{t('tool.ai.style.product')}</option>
							<option value="cinematic">{t('tool.ai.style.cinematic')}</option>
							<option value="editorial">{t('tool.ai.style.editorial')}</option>
							<option value="minimal">{t('tool.ai.style.minimal')}</option>
							<option value="portrait">{t('tool.ai.style.portrait')}</option>
							<option value="anime">{t('tool.ai.style.anime')}</option>
							<option value="watercolor">
								{t('tool.ai.style.watercolor')}
							</option>
							<option value="render3d">{t('tool.ai.style.render3d')}</option>
							<option value="vintage">{t('tool.ai.style.vintage')}</option>
							<option value="cyberpunk">{t('tool.ai.style.cyberpunk')}</option>
							<option value="fantasy">{t('tool.ai.style.fantasy')}</option>
							<option value="popart">{t('tool.ai.style.popart')}</option>
						</select>
					</label>
					<label className="grid gap-2 text-sm font-semibold">
						{t('tool.ai.aspect')}
